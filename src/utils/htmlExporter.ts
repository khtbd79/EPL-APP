...
export async function getStandaloneHTMLContent(currentState: AppState): Promise<string> {
  ...
  const scriptBody = [
    ...
    '      var existing = null;',
    '      try {',
    '        var raw = localStorage.getItem(key) || localStorage.getItem(backupKey);',
    '        if (raw) existing = JSON.parse(raw);',
    '      } catch(e) {}',
    '      var hasUserRecords = existing && typeof existing === "object" && (',
    '        (Array.isArray(existing.matchHistory) && existing.matchHistory.length > 0) ||',
    '        (Array.isArray(existing.eplMatches) && existing.eplMatches.length > 0) ||',
    '        (Array.isArray(existing.marketRecords) && existing.marketRecords.length > 0) ||',
    '        (typeof existing.currentDay === "number" && existing.currentDay > 1)',
    '      );',
    '      if (!hasUserRecords && !existing) {',
    ...
    '      } else if (existing) {',
    '        window.__PRELOADED_APP_STATE__ = existing;',
    '      }',
    '      if (window.indexedDB) {',
    '        try {',
    ...
    "              var store = tx.objectStore('app_state_store');",
    "              var getReq = store.get('current_app_state');",
    '              getReq.onsuccess = function() {',
    '                if (!getReq.result && !hasUserRecords) {',
    "                  store.put(preloaded, 'current_app_state');",
    '                } else if (getReq.result && !hasUserRecords) {',
    '                  window.__PRELOADED_APP_STATE__ = getReq.result;',
    '                }',
    '              };',
    '            } catch(txErr) {}',
    '          };',
    ...
    '    }',
    '  } catch (e) {',
    "    console.error('Initial state setup error:', e);",
    '  }',
    '})();'
  ].join('\n');
